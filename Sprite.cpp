...
BOOL Sprite::IsPointInside(int x, int y) 
{
	...
	pt.x = x;
	pt.y = y;
	return (PtInRect(&m_rcBounds, pt) != 0);
}
...
{
	SetPosition(GetPosition().left + x, GetPosition().top + y);
}
...
inline void Sprite::CalcCollisionRec()
{
...
BOOL Sprite::TestCollision(std::shared_ptr<Sprite> pTestSprite)
{
